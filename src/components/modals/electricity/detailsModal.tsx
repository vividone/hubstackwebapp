...
const DetailsModal: React.FC<FlowProps> = ({ flow, setFlow, data }) => {
            ...
            <div className="flex justify-between">
              <div>
                ...
                <span className="block text-[#3D3066] text-[32px]  font-bold font-openSans">
                  {visibility ? <div className="flex gap-1 items-center"><NairaIcon className="w-[16px]" />{wallet?.balance}</div> : "****"}
                </span>
              </div>
              <div>
          ...
          
        {
          ...
          <div className="flex flex-col gap-1">
            <div className="flex justify-between items-center gap-5">
              ...
              <span className="flex items-center"><NairaIcon className="w-[12px]" />{data?.amount}</span>
            </div>
            
            <div className="flex justify-between items-center gap-5">
              <span className="block">Service Charge</span>
              <span className="flex items-center"><NairaIcon className="w-[12px]" />{data?.surcharge}</span>
            </div>
            
            <div className="flex justify-between items-center gap-5 mb-6">
              <span className="block font-bold">Total</span>
              <span className="flex items-center"><NairaIcon className="w-[12px]" />{data?.amount + data?.surcharge}</span>
            </div>

          </div>
          :
          
          <p className="text-center">The amount of  NGN{data?.amount + data?.surcharge} will be debited from your wallet balance, proceed below to complete transaction </p>
        }
          <div className="flex flex-col gap-4">
            <Button 
              variant="primary" 
              size="full"
              type="submit"
              onClick={() => setFlow("Pay with Wallet")}
            >
